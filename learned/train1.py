...
class ModelArguments:
    base_model_name_or_path: Optional[str] = field(default="show_model/model001",
                                                   metadata={"help": "Path to the base model"})
    lora_weights: Optional[str] = field(default="output_model_user_lora_0705/checkpoint-2790",
                                        metadata={"help": "Path to LoRA weights for fine-tuning"})
...
def load_model_processor(modelargs: ModelArguments):
    model = LlavaForConditionalGeneration.from_pretrained(
        modelargs.base_model_name_or_path,
        torch_dtype=torch.bfloat16,
        low_cpu_mem_usage=True,
    )
    processor = LlavaProcessor.from_pretrained(modelargs.base_model_name_or_path)

    if modelargs.train_type == "use_lora":
        print("加载lora权重")
        model = PeftModel.from_pretrained(model, modelargs.lora_weights)
